...
data Cropped x t a = CS (E t a) (x t a)
                       | CE (E t a) (x t a)
                       | C0 (x t a)
                       | C2 (Cropped (Cropped x) t a)

instance (Trajectory x t a) => Trajectory (Cropped x) t a where
...
cropStart :: Trajectory x t a => E t a -> x t a -> Cropped x t a
...
cropEnd :: Trajectory x t a => E t a -> x t a -> Cropped x t a
cropEnd t1 x = CE t1 x

crop :: Trajectory x t a => E t a -> E t a -> x t a -> Cropped x t a
crop t0 t1 = C2 . cropStart t0 . cropEnd t1

cropStart' :: Trajectory x t a => Maybe (E t a) -> x t a -> Cropped x t a
cropStart' Nothing = C0
cropStart' (Just t0) = cropStart t0
cropEnd'   :: Trajectory x t a => Maybe (E t a) -> x t a -> Cropped x t a
cropEnd'   Nothing = C0
cropEnd'   (Just t1) = cropEnd t1
crop' :: Trajectory x t a => Maybe (E t a) -> Maybe (E t a) -> x t a -> Cropped x t a
